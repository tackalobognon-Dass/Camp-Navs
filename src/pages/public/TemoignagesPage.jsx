    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ padding: '16px 14px 80px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <button onClick={() => setShowForm(!showForm)}
          style={{ width: '100%', background: '#054035', color: '#fff', border: 'none', borderRadius: 14, padding: '14px', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        {temoignages.length > 0 && (
          <div style={{ position: 'relative' }}>
            <svg style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 16, height: 16, color: '#9CA3AF' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" value={recherche} onChange={e => { setRecherche(e.target.value); setPage(1) }}
              placeholder="Rechercher un témoignage..."
              style={{ width: '100%', border: '0.5px solid #E5E7EB', borderRadius: 12, padding: '11px 12px 11px 36px', fontSize: 13, outline: 'none', background: '#fff' }} />
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {affiches.map(t => {
            ...
            return (
              <div key={t.id} style={{ background: '#fff', borderRadius: 16, border: '0.5px solid #F3F4F6', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', padding: 16 }}>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  <div style={{ width: 38, height: 38, borderRadius: '50%', background: t.anonyme ? '#F3F4F6' : '#E1F5EE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: t.anonyme ? '#9CA3AF' : '#054035' }}>{initiale}</span>
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  {t.anonyme && (
                    <span style={{ fontSize: 9, background: '#F3F4F6', color: '#6B7280', borderRadius: 20, padding: '2px 8px', fontWeight: 500 }}>Anonyme</span>
                  )}
                </div>

                {/* Texte */}
                <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, margin: '0 0 12px', fontStyle: 'italic' }}>
                  "{t.contenu}"
                </p>

                {/* Actions partager */}
                <div style={{ display: 'flex', gap: 8, borderTop: '0.5px solid #F9FAFB', paddingTop: 12 }}>
                  <button onClick={() => partagerWhatsApp(t)}
                    style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: '#F0FDF4', color: '#166534', border: '0.5px solid #86EFAC', borderRadius: 10, padding: '8px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
                  ...
                  <button onClick={() => partagerNatif(t)}
                    style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: '#F9FAFB', color: '#6B7280', border: '0.5px solid #E5E7EB', borderRadius: 10, padding: '8px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
