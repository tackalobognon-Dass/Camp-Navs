  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [r1, r2, r3] = await Promise.all([
        supabase.from('documents').select('created_at').limit(20),
        supabase.from('chants').select('created_at').limit(20),
        supabase.from('temoignages').select('created_at').eq('statut', 'approuve').limit(20),
      ])
      const all = [...(ann || []), ...(r1.data || []), ...(r2.data || []), ...(r3.data || [])]
      setNbNotifs(all.filter(i => new Date(i.created_at) > new Date(lastSeen)).length)
    }
    fetchData()
  }, [])
