...
// Détecte une icône selon les mots-clés de la mission
function getMissionIcon(texte) {
  ...
  if (t.includes('rapport') || t.includes('bilan') || t.includes('compte rendu') || t.includes('document') || t.includes('archiv')) {
...
// Détecte si une mission a un titre séparé (format "Titre: description")
function parseLigne(ligne) {
  const idx = ligne.indexOf(':')
  if (idx > 0 && idx < 50) {
    return { titre: ligne.slice(0, idx).trim(), desc: ligne.slice(idx + 1).trim() }
  }
  // Sinon les premiers mots = titre
...
const EMPTY_FORM_ADJ = { missions_adjoint: '' }
...
export default function MembresPage() {
  ...
  const chipStyle = () => ({ flexShrink: 0, padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: `1px solid ${VERT}`, background: VERT, color: '#fff' })
    ...
    <AdminLayout>
      ...
      {showForm && (
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: '14px', marginBottom: 14 }}>
          ...
          <div style={{ marginBottom: 8 }}><label style={labelStyle}>Nom complet *</label><input type="text" value={form.nom_complet} onChange={e => setF('nom_complet', e.target.value)} placeholder="Ex : N'DRI SERGE PACOME" style={inputStyle} /></div>
          <div style={{ marginBottom: 8 }}><label style={labelStyle}>Rôle *</label><input type="text" value={form.role} onChange={e => setF('role', e.target.value)} placeholder="Ex : Directeur du camp" style={inputStyle} /></div>
          <div style={{ marginBottom: 8 }}><label style={labelStyle}>Commission</label><input type="text" value={form.commission} onChange={e => setF('commission', e.target.value)} placeholder="Ex : Logistique, Louange..." style={inputStyle} /></div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div><label style={labelStyle}>Téléphone</label><input type="text" value={form.telephone} onChange={e => setF('telephone', e.target.value)} placeholder="07 XX XX XX XX" style={inputStyle} /></div>
            <div><label style={labelStyle}>Email</label><input type="email" value={form.email} onChange={e => setF('email', e.target.value)} placeholder="email@exemple.com" style={inputStyle} /></div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={labelStyle}>Missions & Responsabilités</label>
            <p style={{ fontSize: 10, color: '#94A3B8', margin: '0 0 6px' }}>Une mission par ligne. Format : "Titre : description" pour séparer titre et détail.</p>
            <textarea value={form.missions} onChange={e => setF('missions', e.target.value)}
              placeholder={`Ex :\nGestion financière : Superviser le budget et les recettes du camp.\nSuivi des inscriptions : Valider et mettre à jour les dossiers campeurs.`}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button type="button" onClick={handleSave} disabled={saving || !form.nom_complet || !form.role} style={{ flex: 1, background: VERT, color: '#fff', border: 'none', borderRadius: 10, padding: '10px', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: saving || !form.nom_complet || !form.role ? 0.7 : 1 }}>{saving ? '...' : editId ? 'Modifier' : 'Ajouter'}</button>
          </div>
        </div>
      )}
                ...
                {menuOuvert === m.id && (
                  <div style={{ position: 'absolute', right: 0, top: 30, background: '#fff', borderRadius: 10, border: '1px solid #E2E8F0', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', zIndex: 20, minWidth: 150, overflow: 'hidden' }}>
                    {[
                      ...
                      <div key={item.label}>
                        <button type="button" onClick={item.action} style={{ width: '100%', padding: '9px 14px', fontSize: 13, color: item.color, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>{item.label}</button>
      ...
      {ficheOuverte && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: '#F8FAFC', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', paddingBottom: 28 }}
            ...
            {/* En-tête profil */}
            <div style={{ background: VERT, margin: '14px 14px 0', borderRadius: 16, padding: '16px', display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            {(ficheOuverte.telephone || ficheOuverte.email) && (
              <div style={{ margin: '10px 14px 0', background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: '10px 14px', display: 'flex', gap: 16 }}>
            ...
            <div style={{ margin: '14px 14px 0' }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.1em', margin: '0 0 10px', textTransform: 'uppercase' }}>
                Missions & Responsabilités
              </p>
              ...
              {parseMissions(ficheOuverte.missions).length === 0 ? (
                <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: '20px', textAlign: 'center' }}>
                  ...
                  <button type="button" onClick={() => { setFicheOuverte(null); openEdit(ficheOuverte) }}
                    style={{ fontSize: 12, fontWeight: 600, color: VERT, background: 'none', border: 'none', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {parseMissions(ficheOuverte.missions).map((ligne, i) => {
                    ...
                    const icon = getMissionIcon(ligne)
                    return (
            ...
            <div style={{ display: 'flex', gap: 10, margin: '14px 14px 0' }}>
              <button type="button" onClick={() => { setFicheOuverte(null); openEdit(ficheOuverte) }}
                style={{ flex: 1, background: VERT_CLAIR, color: VERT, border: 'none', borderRadius: 10, padding: '11px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
